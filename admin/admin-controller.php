...
function  WBSMSC_handleSubmitIntegration($data){
   global $wpOption;
    global $wpdb;    
    $statuses = (array) get_terms( 'shop_order_status', array( 'hide_empty' => 0, 'orderby' => 'id' ) );
    ...
    foreach($statuses as $ks => $orderStatus){
        ...
        $arrSEtting[$orderStatus->slug.'Custom'] =   empty($data['burstSms'.$orderStatus->slug.'Custom'])?'':trim($data['burstSms'.$orderStatus->slug.'Custom']);
        $arrSEtting['ena'.ucfirst($orderStatus->slug).'Custom'] =  empty($data['WB_ena'.ucfirst($orderStatus->slug).'Custom'])?'':$data['WB_ena'.ucfirst($orderStatus->slug).'Custom'];
