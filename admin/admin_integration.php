...
class TranmsitsSMS_Integration extends WC_Integration {
	 ...
	 */
	   ...
	   //	$this->method_description = __( 'Transmit SMS Notifications.', 'transmitsSMS-integration' );
        ...
        // Actions.
        add_action( 'admin_init', array(&$this,'WBjQueryUi' ));
        add_action('admin_footer', array(&$this,'WB_admin_footer_function'));
    	// Filters.
    ...
    public function process_admin_options(){
        if(!$this->validate_phone_number('WB_adminNumber')){
            ...
            WBSMSC_handleSubmitIntegration($data);
	...
	public function init_form_fields() {
        ...
        foreach($statuses as $ks => $orderStatus){
            ...
            $textAreacss = '';
            if(!empty($WBSms['ena'.ucfirst($orderStatus->slug).'Custom'])){
            ...
            /*array(
                                ...
                                'checked' =>'checked', */
            $this->form_fields['WB_ena'.ucfirst($orderStatus->slug).'Custom'] = array(
           	                
                            'label'             => __('Enable processing '.$orderStatus->slug.' notifications ', 'woocommerce' ),
            				'type' 				=> 'checkbox',
            ...
            if($ks == 0){
                $this->form_fields['WB_ena'.ucfirst($orderStatus->slug).'Custom']['title'] = 'Notifications' ;
            }
            $this->form_fields[$defvar] = array(
				'title'             => __( 'Order '.$orderStatus->slug.' custom message : ', 'woocommerce' ),
                 'default'      => $$defvar,
				'type'              => 'textarea',
				'description'       => __( 'This message would be sent to the customer.  avalible shortcode : <kbd>[order_billing_country]</kbd>, 
                                    ...
                                    <kbd> [order_billing_city]</kbd>, <kbd>[order_billing_state]</kbd>, <kbd>[order_billing_postcode]</kbd>, 
                                     <kbd>[order_billing_country]</kbd>','woocommerce' ),
           ...
           'addToPCategory' => array(
            ...
            'WB_receivedCustom' => array(
				...
				'description'       => __( 'This message would be sent to the admin.  avalible shortcode :<kbd>[order_billing_country]</kbd>,
                                      ...
                                      <kbd>[order_billing_postcode]</kbd>, <kbd style="cursor:pointer">[order_billing_country]</kbd>','woocommerce' ),
                //'desc_tip'          => true,
